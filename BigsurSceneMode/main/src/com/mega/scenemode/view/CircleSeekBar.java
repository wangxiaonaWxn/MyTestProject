...
public class CircleSeekBar extends View {
    ...
    private static final String INSTANCE_WHEEL_SCROLL_ONLY_ONE_CIRCLE
            = "wheel_scroll_only_one_circle";
    ...
    private boolean mIsHasReachedCornerRound;
    ...
    private boolean mIsHasWheelShadow;
    private boolean mIsHasPointerShadow;
    ...
    private boolean mIsScrollOneCircle;
    ...
    private void initPaints() {
        ...
        if (mIsHasWheelShadow) {
            mWheelPaint.setShadowLayer(mWheelShadowRadius, mDefShadowOffset,
        ...
        if (mIsHasPointerShadow) {
            mPointerPaint.setShadowLayer(mPointerShadowRadius, mDefShadowOffset,
                    mDefShadowOffset, Color.DKGRAY);
        }
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        ...
        mMaxProcess = typedArray.getInt(R.styleable.CircleSeekBar_wheel_max_process, 100);
        mCurProcess = typedArray.getInt(R.styleable.CircleSeekBar_wheel_cur_process, 0);
        if (mCurProcess > mMaxProcess) {
            mCurProcess = mMaxProcess;
        }
        mReachedColor = typedArray.getColor(R.styleable.CircleSeekBar_wheel_reached_color,
                getColor(R.color.def_reached_color));
        mUnreachedColor = typedArray.getColor(R.styleable.CircleSeekBar_wheel_unreached_color,
                getColor(R.color.def_wheel_color));
        mUnreachedWidth = typedArray.getDimension(R.styleable.CircleSeekBar_wheel_unreached_width,
                getDimen(R.dimen.def_wheel_width));
        mIsHasReachedCornerRound = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_reached_has_corner_round, true);
        mReachedWidth = typedArray.getDimension(
                R.styleable.CircleSeekBar_wheel_reached_width, mUnreachedWidth);
        mPointerColor = typedArray.getColor(
                R.styleable.CircleSeekBar_wheel_pointer_color, getColor(R.color.def_pointer_color));
        mPointerRadius = typedArray.getDimension(
                R.styleable.CircleSeekBar_wheel_pointer_radius, mReachedWidth / 2);
        mIsHasWheelShadow = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_has_wheel_shadow, false);
        if (mIsHasWheelShadow) {
            mWheelShadowRadius = typedArray.getDimension(
                    R.styleable.CircleSeekBar_wheel_shadow_radius,
                    getDimen(R.dimen.def_shadow_radius));
        }
        mIsHasPointerShadow = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_has_pointer_shadow, false);
        if (mIsHasPointerShadow) {
            mPointerShadowRadius = typedArray.getDimension(
                    R.styleable.CircleSeekBar_wheel_pointer_shadow_radius,
                    getDimen(R.dimen.def_shadow_radius));
        }
        mIsHasCache = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_has_cache, mIsHasWheelShadow);
        mIsCanTouch = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_can_touch, true);
        mIsScrollOneCircle = typedArray.getBoolean(
                R.styleable.CircleSeekBar_wheel_scroll_only_one_circle, false);

        if (mIsHasPointerShadow | mIsHasWheelShadow) {
            setSoftwareLayer();
        }
        typedArray.recycle();
    }
    ...
    private void initPadding() {
        ...
        int maxPadding = Math.max(paddingLeft, Math.max(paddingTop,
                Math.max(paddingRight, Math.max(paddingBottom,
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        float wheelRadius = (getWidth() - getPaddingLeft() - getPaddingRight()) / 2
                - mUnreachedWidth / 2;
        ...
        MLog.d("onDraw ------------------- mRectF : " + mRectF);
        MLog.d("onDraw ------------------- mCurAngle : " + mCurAngle);
        MLog.d("onDraw ------------------- mReachedPaint : " + mReachedPaint);
        //画选中区域
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        float x = event.getX();
        float y = event.getY();
        if (mIsCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE || isTouch(x, y))) {
            // 通过当前触摸点搞到cos角度值
            ...
            // onTouch 时，是主动滑动进度。 小憩模式规定不可以将时间设置为 0，这里对应成角度不可以小于 3
            if (mCurAngle < 3 && mCurAngle > 0) {
                mCurAngle = 3;
            }

            MLog.d("---------------------------- Angle : " + mCurAngle);
            mCurProcess = getSelectedValue();
            refershWheelCurPosition(cos);
            if (mChangListener != null && (event.getAction()
                    & (MotionEvent.ACTION_MOVE | MotionEvent.ACTION_UP)) > 0) {
    ...
    private void refershUnreachedWidth() {
        mUnreachedRadius = (getMeasuredWidth() - getPaddingLeft()
                - getPaddingRight() - mUnreachedWidth) / 2;
    }
    ...
    protected @NonNull Parcelable onSaveInstanceState() {
        ...
        bundle.putBoolean(INSTANCE_REACHED_CORNER_ROUND, mIsHasReachedCornerRound);
        ...
        bundle.putBoolean(INSTANCE_POINTER_SHADOW, mIsHasPointerShadow);
        bundle.putFloat(INSTANCE_POINTER_SHADOW_RADIUS, mPointerShadowRadius);
        bundle.putBoolean(INSTANCE_WHEEL_SHADOW, mIsHasWheelShadow);
        bundle.putFloat(INSTANCE_WHEEL_SHADOW_RADIUS, mPointerShadowRadius);
        bundle.putBoolean(INSTANCE_WHEEL_HAS_CACHE, mIsHasCache);
        bundle.putBoolean(INSTANCE_WHEEL_CAN_TOUCH, mIsCanTouch);
        bundle.putBoolean(INSTANCE_WHEEL_SCROLL_ONLY_ONE_CIRCLE, mIsScrollOneCircle);
        return bundle;
    }
    ...
    protected void onRestoreInstanceState(@NonNull Parcelable state) {
        if (state instanceof Bundle) {
            ...
            mIsHasReachedCornerRound = bundle.getBoolean(INSTANCE_REACHED_CORNER_ROUND);
            ...
            mIsHasPointerShadow = bundle.getBoolean(INSTANCE_POINTER_SHADOW);
            mPointerShadowRadius = bundle.getFloat(INSTANCE_POINTER_SHADOW_RADIUS);
            mIsHasWheelShadow = bundle.getBoolean(INSTANCE_WHEEL_SHADOW);
            mPointerShadowRadius = bundle.getFloat(INSTANCE_WHEEL_SHADOW_RADIUS);
            mIsHasCache = bundle.getBoolean(INSTANCE_WHEEL_HAS_CACHE);
            mIsCanTouch = bundle.getBoolean(INSTANCE_WHEEL_CAN_TOUCH);
            mIsScrollOneCircle = bundle.getBoolean(INSTANCE_WHEEL_SCROLL_ONLY_ONE_CIRCLE);
            initPaints();
        } else {
    ...
    public boolean isHasReachedCornerRound() {
        return mIsHasReachedCornerRound;
    }

    public void setHasReachedCornerRound(boolean hasReachedCornerRound) {
        mIsHasReachedCornerRound = hasReachedCornerRound;
    ...
    public boolean ismIsHasWheelShadow() {
        return mIsHasWheelShadow;
    }

    public void setWheelShadow(float wheelShadow) {
        this.mWheelShadowRadius = wheelShadow;
        if (wheelShadow == 0) {
            mIsHasWheelShadow = false;
    ...
    public boolean ismIsHasPointerShadow() {
        return mIsHasPointerShadow;
    }
    ...
    public interface OnSeekBarChangeListener {
        void onChanged(@NonNull CircleSeekBar seekbar, int curValue);
    }
}